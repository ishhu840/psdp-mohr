...
def app():
    ...
    st.title(":bar_chart: PSDP (08) on Going Projects (Budget Execution) FY 2024-25")
    st.markdown("#")
    ...
    with left_column:
        ...
        sizes = [max(total_budget - released_budget, 0), released_budget]  # Ensure non-negative values
        explode = (0, 0.1)
    ...
    with right_column:
        ...
        expenditure_percentage = (total_expenditure / released_budget) * 100 if released_budget > 0 else 0

        # Handle cases where expenditure exceeds released budget
        ...
        ax2.pie(sizes, explode=explode, labels=labels, autopct='%1.01f%%',
                shadow=False, startangle=60)
